...
class AktualizrSecondaryWrapper {
 public:
  AktualizrSecondaryWrapper() {
    ...
    secondary_ = std::make_shared<AktualizrSecondaryFile>(config, _storage, update_agent);
  }
  ...
  Uptane::Target getPendingVersion() const {
    ...
    _storage->loadInstalledVersions(secondary_->serial().ToString(), nullptr, &pending_target);
